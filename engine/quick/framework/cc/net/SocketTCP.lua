...
function SocketTCP:_onConnected()
	...
	local __tick = function()
		while true do
			local now = SocketTCP.getTime()
			local cha = now - self.lastTime
			if cha>WAITTIMEOUT then
				self:setLastTime()
...
function SocketTCP:setLastTime()
	self.lastTime = SocketTCP.getTime()
end

function SocketTCP:setWaitTimeOut(time)
	WAITTIMEOUT = time or 10
end
